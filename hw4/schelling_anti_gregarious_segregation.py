...
# Schelling segregation model and anti-gregarious segregation model. To change between them, change mode in main().

SCHELLING = 1
ANTI_GREGARIOUS = -1


def get_majority(town: npt.NDArray[npt.NDArray[int]]) -> npt.NDArray[npt.NDArray[int]]:
    size = 3
    convolution_kernel = np.ones((size, size)) / (size ** 2)
    convolution = fftconvolve(town, convolution_kernel, mode='same')
    return (convolution > 0).astype(int) - (convolution < 0).astype(int)


def calculate_happiness(town: npt.NDArray[npt.NDArray[int]], mode: int = SCHELLING) -> (float, float, float):
    happiness = get_majority(town) * town * mode
...
def main():
    ...
    for t in trange(num_rounds):
        happiness_a[t], happiness_b[t], avg_happiness[t] = calculate_happiness(town, mode)

        happiness = get_majority(town) * town * mode  # mode inverts if anti-gregarious
    ...
    plt.suptitle('Schelling' if mode == SCHELLING else 'Anti-gregarious')
